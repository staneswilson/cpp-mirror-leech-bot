...
[[nodiscard]] cmlb::core::Result<cmlb::domain::UploadDestination>
resolve_destination(const MirrorRequest& req,
...
MirrorUrl::execute(MirrorRequest request) {
    cmlb::core::Logger::info(
        "mirror_url: starting user={} chat={} backend={} url={}",
    ...
    if (!settings_res) co_return std::unexpected(settings_res.error());

    auto dest_res = resolve_destination(request, *settings_res);
    if (!dest_res) co_return std::unexpected(dest_res.error());
    const auto destination = *dest_res;

    auto& downloader =
        request.use_qbittorrent ? qbit_ : aria2_;
    auto& uploader =
        (destination == cmlb::domain::UploadDestination::Rclone) ? rclone_
                                                                 : gdrive_;
    ...
    auto status_msg = co_await messenger_.send_html(
        ...
        fmt::format("<b>Queued</b>: <code>{}</code>",
                    cmlb::core::escape_html(
                        cmlb::core::truncate_for_display(request.url, 200))));
    if (!status_msg) co_return std::unexpected(status_msg.error());
    ...
    auto persist_best_effort = [&, task_id = meta.id.value()](
                                   std::string_view stage)
        -> asio::awaitable<void> {
        auto saved = co_await tasks_.save(task);
        if (!saved) {
            cmlb::core::Logger::warn(
                "mirror_url: task={} persist after {} failed: {}",
                task_id, stage, saved.error().message);
        }
        co_return;
    };
    ...
    auto fail_task = [&](std::string_view stage, cmlb::core::AppError err)
        -> asio::awaitable<cmlb::core::Result<cmlb::domain::TaskId>> {
        cmlb::core::Logger::error(
            "mirror_url: failed task={} stage={} code={} msg={}",
            meta.id.value(), stage,
            cmlb::core::error_code_name(err.code), err.message);
        ...
        const std::string html = fmt::format(
            ...
            cmlb::core::escape_html(
                cmlb::core::truncate_for_display(err.message, 512)));
    ...
    auto fail_with = [&](std::string_view       stage,
                         cmlb::core::ErrorCode  code,
                         std::string            message)
        -> asio::awaitable<cmlb::core::Result<cmlb::domain::TaskId>> {
        co_return co_await fail_task(
            stage, cmlb::core::AppError{code, std::move(message)});
    };
    ...
    const auto downloader_kind = request.use_qbittorrent
        ? cmlb::domain::DownloaderKind::Qbittorrent
    ...
    if (settings_res->has_value()) {
        ...
        if (destination == cmlb::domain::UploadDestination::GoogleDrive
            && s.gdrive_folder_id) {
            up_cfg.folder_id = *s.gdrive_folder_id;
        }
        if (destination == cmlb::domain::UploadDestination::Rclone
            && s.rclone_remote) {
    ...
    auto spawn_upload = [&, uploader_ptr](std::filesystem::path file_path) {
        {
            ...
            ++state->in_flight;
            state->upload_signals.push_back(
                std::make_unique<asio::cancellation_signal>());
        }
        auto signal_slot =
            state->upload_signals.back()->slot();
        asio::co_spawn(
            executor_.get_executor(),
            [state, uploader_ptr, file_path, up_cfg]()
                -> asio::awaitable<void> {
                auto res = co_await uploader_ptr->upload_file(
                    file_path, up_cfg, {});
                {
                    std::lock_guard lk{state->mu};
                    --state->in_flight;
    ...
    auto drain_uploads = [&]() -> asio::awaitable<void> {
        ...
        while (true) {
            {
                std::lock_guard lk{state->mu};
                if (state->in_flight == 0) co_return;
            }
    ...
    while (true) {
        ...
        const bool coroutine_cancelled =
        ...
        if (coroutine_cancelled || registry_cancelled) {
            ...
            (void)co_await messenger_.edit_html(
                request.chat, *status_msg, "<b>Cancelled</b>");
            cmlb::core::Logger::info(
                "mirror_url: task={} cancelled (source={})",
                meta.id.value(),
        ...
        if (!download_complete) {
            auto status = co_await cmlb::core::with_timeout(
                downloader.status(gid), kStatusRpcTimeout);
            if (!status) {
                cancel_all_uploads();
                co_await drain_uploads();
            ...
            if (final_status.state == download_ns::DownloadState::Error) {
                cancel_all_uploads();
                co_await drain_uploads();
                const std::string msg = final_status.error_message.value_or(
                    std::string{"downloader reported error"});
                co_return co_await fail_with(
                    "download", cmlb::core::ErrorCode::Aria2Rpc, msg);
            }
            if (final_status.state == download_ns::DownloadState::Removed) {
                cancel_all_uploads();
                co_await drain_uploads();
                co_return co_await fail_with(
                    "download", cmlb::core::ErrorCode::Cancelled,
                    "download removed externally");
            }
            ...
            (void)co_await progress_renderer_.render(
                request.chat,
                std::span<const download_ns::DownloadStatus>{active});
        }
        ...
        if (any_upload_failed) {
            cancel_all_uploads();
            co_await drain_uploads();
            cmlb::core::AppError upload_err{cmlb::core::ErrorCode::Unknown,
                                            "upload failed"};
            {
                std::lock_guard lk{state->mu};
                if (state->first_error) upload_err = *state->first_error;
            }
        ...
        timer.expires_after(download_complete ? kCoordinationTick
                                              : kPollInterval);
    ...
    if (!any_uploaded) {
        ...
        if (final_status.save_path) {
            ...
            auto file_res = co_await uploader.upload_file(
                final_status.files.front(), up_cfg, {});
            if (!file_res) {
    ...
    (void)co_await downloader.remove(gid, false);
    {
        ...
        const std::string& outcome = upload_outcome.link.empty()
            ? upload_outcome.file_id
            : upload_outcome.link;
        (void)co_await messenger_.edit_html(
            request.chat, *status_msg,
            fmt::format("<b>Completed</b>: <code>{}</code>",
                        cmlb::core::escape_html(
                            cmlb::core::truncate_for_display(outcome, 400))));
    }
    cmlb::core::Logger::info("mirror_url: task={} completed",
                             meta.id.value());
    co_return meta.id;
}
