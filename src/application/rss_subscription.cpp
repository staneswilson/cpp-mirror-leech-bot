...
asio::awaitable<cmlb::core::Result<std::int64_t>>
RssSubscription::add(pers_ns::RssFeed feed) {
    cmlb::core::Logger::info("rss_subscription: add chat={} url={}",
                             feed.chat.value(), feed.url);
    ...
    cmlb::core::Logger::info("rss_subscription: added feed_id={}",
                             saved->feed_id);
    co_return saved->feed_id;
}

asio::awaitable<cmlb::core::Result<std::vector<pers_ns::RssFeed>>>
RssSubscription::list_for_chat(cmlb::domain::ChatId chat) {
    cmlb::core::Logger::info("rss_subscription: list chat={}", chat.value());

    auto all = co_await repo_.list_enabled();
    if (!all) co_return std::unexpected(all.error());
...
RssSubscription::remove(std::int64_t feed_id,
    ...
    if (!loaded->has_value()) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::NotFound,
                                    "rss remove: feed not found");
    }
    if (loaded->value().chat != requester_chat) {
        cmlb::core::Logger::warn(
            "rss_subscription: remove denied feed_id={} requester={} "
    ...
    if (!removed) co_return std::unexpected(removed.error());
