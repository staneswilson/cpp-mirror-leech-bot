...
asio::awaitable<cmlb::core::Result<void>> PauseTask::execute(PauseTaskRequest request) {
    cmlb::core::Logger::info(
        "pause_task: task={} chat={}", request.task_id.value(), request.chat.value());

    auto loaded = co_await tasks_.find(request.task_id);
    if (!loaded)
        co_return std::unexpected(loaded.error());
    if (!loaded->has_value()) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::NotFound, "pause: task not found");
    }
    ...
    if (!did.has_value() || kind == cmlb::domain::DownloaderKind::None) {
        cmlb::core::Logger::warn("pause_task: task={} has no downloader binding yet",
                                 request.task_id.value());
        co_return cmlb::core::error(cmlb::core::ErrorCode::InvalidState,
    ...
    if (!backend) {
        cmlb::core::Logger::warn("pause_task: backend refused: {}", backend.error().message);
