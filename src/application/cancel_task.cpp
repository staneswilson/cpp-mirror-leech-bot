...
CancelTask::execute(CancelTaskRequest request) {
    cmlb::core::Logger::info("cancel_task: task={} chat={}",
    ...
    if (!loaded->has_value()) {
        cmlb::core::Logger::warn("cancel_task: task not found id={}",
                                 request.task_id.value());
    ...
    if (task.is_terminal()) {
        cmlb::core::Logger::warn("cancel_task: task={} already terminal",
                                 request.task_id.value());
        co_return cmlb::core::error(
    ...
    if (active_tasks_.cancel(request.task_id)) {
        cmlb::core::Logger::info(
            "cancel_task: task={} signalled in-flight coroutine",
            request.task_id.value());
        (void)co_await messenger_.send_html(
            request.chat,
            fmt::format("<b>Cancelling</b>: <code>{}</code>",
                        request.task_id.value()));
    ...
    if (did.has_value()) {
        switch (kind) {
            ...
            case cmlb::domain::DownloaderKind::None:
                ...
                cmlb::core::Logger::warn(
                    "cancel_task: task={} has gid but DownloaderKind::None",
                    request.task_id.value());
                break;
        }
    } else {
        cmlb::core::Logger::info(
            "cancel_task: task={} has no downloader binding; "
            "cancelling without backend remove",
            request.task_id.value());
    }

    if (auto mark = task.mark_cancelled(); !mark) {
        cmlb::core::Logger::error("cancel_task: state transition failed: {}",
                                  mark.error().message);
    ...
    (void)co_await messenger_.send_html(
        request.chat,
        fmt::format("<b>Cancelled</b>: <code>{}</code>",
                    request.task_id.value()));
    cmlb::core::Logger::info("cancel_task: task={} cancelled",
                             request.task_id.value());
    co_return cmlb::core::Result<void>{};
}

asio::awaitable<cmlb::core::Result<CancelAllResult>>
CancelTask::cancel_all(cmlb::domain::ChatId chat) {
    cmlb::core::Logger::info("cancel_all: chat={}", chat.value());

    auto incomplete = co_await tasks_.incomplete();
    if (!incomplete) co_return std::unexpected(incomplete.error());
    ...
    for (auto& task : *incomplete) {
        ...
        if (outcome.has_value()) {
            ...
            ++result.failed;
            cmlb::core::Logger::warn(
                "cancel_all: task={} failed: {}",
                task.metadata().id.value(), outcome.error().message);
        }
    }
    ...
    cmlb::core::Logger::info("cancel_all: chat={} cancelled={} failed={}",
                             chat.value(), result.cancelled, result.failed);
    co_return result;
}
