...
ResumeTask::execute(ResumeTaskRequest request) {
    cmlb::core::Logger::info("resume_task: task={} chat={}",
    ...
    if (!did.has_value() || kind == cmlb::domain::DownloaderKind::None) {
        cmlb::core::Logger::warn(
            "resume_task: task={} has no downloader binding yet",
            request.task_id.value());
        co_return cmlb::core::error(
            cmlb::core::ErrorCode::InvalidState,
    ...
    if (!backend) {
        cmlb::core::Logger::warn("resume_task: backend refused: {}",
    ...
    (void)co_await messenger_.send_html(
        request.chat,
        fmt::format("<b>Resumed</b>: <code>{}</code>",
                    request.task_id.value()));
