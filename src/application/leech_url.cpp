...
LeechUrl::execute(LeechRequest request) {
    cmlb::core::Logger::info(
        "leech_url: starting user={} chat={} backend={} url={}",
    ...
    auto persist_best_effort = [&, task_id = meta.id.value()](
        ...
        if (!saved) {
            cmlb::core::Logger::warn(
                "leech_url: task={} persist after {} failed: {}",
    ...
    auto fail_task = [&](std::string_view stage, cmlb::core::AppError err)
        ...
        cmlb::core::Logger::error(
            "leech_url: failed task={} stage={} code={} msg={}",
            meta.id.value(), stage,
    ...
    while (true) {
        ...
        if (coroutine_cancelled || registry_cancelled) {
            ...
            cmlb::core::Logger::info(
                "leech_url: task={} cancelled (source={})",
                meta.id.value(),
        ...
        if (any_upload_failed) {
            cancel_all_uploads();
            co_await drain_uploads();
            cmlb::core::AppError upload_err{
                cmlb::core::ErrorCode::Unknown, "upload failed"};
            {
                std::lock_guard lk{state->mu};
                if (state->first_error) {
    ...
    if (completed_count == 0) {
        ...
        if (final_status.save_path) {
            auto dir_res = co_await telegram_uploader_.upload_directory(
            ...
            if (dir_res->empty()) {
                co_return co_await fail_with(
                    "upload", cmlb::core::ErrorCode::Internal,
                    "upload produced no results");
            }
        } else {
            auto file_res = co_await telegram_uploader_.upload_file(
    ...
    (void)co_await downloader.remove(gid, false);
    (void)co_await messenger_.edit_html(
        request.chat, *status_msg, "<b>Completed</b>");
    cmlb::core::Logger::info("leech_url: task={} completed",
                             meta.id.value());
    co_return meta.id;
}
