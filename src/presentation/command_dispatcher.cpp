...
CommandDispatcher::dispatch(CommandRequest request) {
    ...
    if (entry_it == commands_.end()) {
        Logger::debug("dispatch: unknown command \"{}\" from user {}",
                      request.command, request.sender.value());
    ...
    if (!deps_.authority.can_run(request.sender, request.chat, entry.required)) {
        Logger::info("dispatch: permission denied for \"{}\" (user {} chat {})",
                     resolved_name, request.sender.value(), request.chat.value());
        ...
        const auto required_label = [](Permission p) -> std::string_view {
            switch (p) {
                ...
                case Permission::User:   return "authorized user";
                case Permission::Admin:  return "admin (sudo)";
                case Permission::Owner:  return "owner";
            }
            return "authorized user";
        }(entry.required);
        const std::string notice =
            "<b>Permission denied.</b> <code>/" +
...
void CommandDispatcher::register_builtins_() {
    ...
    const auto require_arg =
        [this](CommandRequest& req, std::string_view usage_html)
            -> asio::awaitable<bool> {
            if (!req.arguments.empty()) co_return true;
            (void)co_await deps_.messenger.send_html(
                req.chat, std::string{usage_html});
            co_return false;
        };

    // -------- mirror / qbmirror -------------------------------------------
    register_("mirror", Permission::User,
        ...
        [this, require_arg](CommandRequest req) -> asio::awaitable<Result<void>> {
            if (!co_await require_arg(req,
                    "<b>Usage:</b> <code>/mirror &lt;url|magnet&gt;</code>")) {
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/mirror requires a url");
            }
            MirrorRequest mr{
    ...
    register_("qbmirror", Permission::User,
        "Mirror via qBittorrent (use for torrents you intend to seed).",
        [this, require_arg](CommandRequest req) -> asio::awaitable<Result<void>> {
            if (!co_await require_arg(req,
                    "<b>Usage:</b> <code>/qbmirror &lt;magnet|torrent_url&gt;</code>")) {
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/qbmirror requires a magnet/url");
            }
            MirrorRequest mr{
                .url             = std::move(req.arguments),
                .user            = req.sender,
    ...
    // -------- leech / qbleech ---------------------------------------------
    register_("leech", Permission::User,
        "Download via aria2 and upload back to this chat.",
        [this, require_arg](CommandRequest req) -> asio::awaitable<Result<void>> {
            if (!co_await require_arg(req,
                    "<b>Usage:</b> <code>/leech &lt;url|magnet&gt;</code>")) {
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/leech requires a url");
            }
            LeechRequest lr{
                .url             = std::move(req.arguments),
                .user            = req.sender,
    ...
    register_("qbleech", Permission::User,
        "Leech via qBittorrent (use for torrents you intend to seed).",
        [this, require_arg](CommandRequest req) -> asio::awaitable<Result<void>> {
            if (!co_await require_arg(req,
                    "<b>Usage:</b> <code>/qbleech &lt;magnet|torrent_url&gt;</code>")) {
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/qbleech requires a magnet/url");
            }
            LeechRequest lr{
                .url             = std::move(req.arguments),
                .user            = req.sender,
    ...
    // -------- clone / count / del -----------------------------------------
    register_("clone", Permission::User,
        "Server-side copy a GDrive file or folder to the configured parent.",
        [this, require_arg](CommandRequest req) -> asio::awaitable<Result<void>> {
            if (!co_await require_arg(req,
                    "<b>Usage:</b> <code>/clone &lt;drive_url&gt;</code>")) {
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/clone requires a drive url");
            }
            CloneRequest cr{
            ...
            auto send = co_await deps_.messenger.send_html(
                req.chat,
                "<b>Clone:</b> <code>" +
                    HtmlRenderer::escape_html(*result) + "</code>");
            if (!send) co_return std::unexpected(send.error());
            co_return Result<void>{};
        });

    register_("count", Permission::User,
        "Recursively count files/folders/bytes in a GDrive resource.",
        [this, require_arg](CommandRequest req) -> asio::awaitable<Result<void>> {
            if (!co_await require_arg(req,
                    "<b>Usage:</b> <code>/count &lt;drive_url&gt;</code>")) {
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/count requires a drive url");
            }
            CountRequest cr{
                .source_url = std::move(req.arguments),
                .user       = req.sender,
            ...
            const std::string body =
                "<b>Count</b>\n"
                "<b>Files:</b> <code>" + std::to_string(result->files) + "</code>\n"
                "<b>Folders:</b> <code>" + std::to_string(result->folders) + "</code>\n"
                "<b>Total:</b> <code>" +
    ...
    register_("del", Permission::Admin,
        "Delete a GDrive file or folder (trash, recoverable for 30 days).",
        [this, require_arg](CommandRequest req) -> asio::awaitable<Result<void>> {
            if (!co_await require_arg(req,
                    "<b>Usage:</b> <code>/del &lt;drive_url&gt;</code>")) {
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/del requires a drive url");
            }
            ...
            if (!result) co_return std::unexpected(result.error());
            auto send = co_await deps_.messenger.send_html(
                req.chat, "<b>Deleted.</b>");
            if (!send) co_return std::unexpected(send.error());
            co_return Result<void>{};
        });

    // -------- cancel / cancelall ------------------------------------------
    register_("cancel", Permission::User,
        ...
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            ...
            if (split.head.empty()) {
                auto send = co_await deps_.messenger.send_html(
                    req.chat,
                    "<b>Usage:</b> <code>/cancel &lt;task_id&gt;</code>");
                (void)send;
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/cancel requires a task id");
            }
    ...
    register_("cancelall", Permission::Admin,
    ...
    // -------- pause / resume ----------------------------------------------
    register_("pause", Permission::User,
        ...
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            ...
            if (split.head.empty()) {
                auto send = co_await deps_.messenger.send_html(
                    req.chat,
                    "<b>Usage:</b> <code>/pause &lt;task_id&gt;</code>");
                (void)send;
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/pause requires a task id");
            }
            PauseTaskRequest pr{
                .task_id = cmlb::domain::TaskId{split.head},
                .chat    = req.chat,
    ...
    register_("resume", Permission::User,
        ...
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            ...
            if (split.head.empty()) {
                auto send = co_await deps_.messenger.send_html(
                    req.chat,
                    "<b>Usage:</b> <code>/resume &lt;task_id&gt;</code>");
                (void)send;
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "/resume requires a task id");
            }
    ...
    register_("settings", Permission::Anyone,
        ...
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            ...
            UpdateUserSettingsRequest read_req{
                .user   = req.sender,
                .mutate = [](auto&) noexcept {},
            };
            auto settings = co_await deps_.update_user.execute(std::move(read_req));
            if (!settings) co_return std::unexpected(settings.error());
    ...
    register_("botsettings", Permission::Admin,
    ...
    auto help_handler =
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            ...
            for (const auto& [name, entry] : commands_) {
                ...
                // distraction.
                if (!deps_.authority.can_run(
                        req.sender, req.chat, entry.required)) {
                    continue;
                }
            ...
            std::ranges::sort(descriptions,
                [](const auto& a, const auto& b) {
                    if (a.permission != b.permission) {
                        return static_cast<int>(a.permission) <
                               static_cast<int>(b.permission);
                    }
    ...
    register_("help",  Permission::Anyone, "Show this list of commands.",
              help_handler);
    register_("start", Permission::Anyone,
        ...
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            ...
            const std::string body =
                HtmlRenderer::render_greeting(std::string_view{});
            auto send = co_await deps_.messenger.send_html(req.chat, body);
            if (!send) co_return std::unexpected(send.error());
            co_return Result<void>{};
        });
    ...
    // -------- stats -------------------------------------------------------
    register_("stats", Permission::Anyone,
    ...
    // -------- version -----------------------------------------------------
    register_("version", Permission::Anyone,
        "Show the running cmlb build version.",
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            const std::string body =
                "<b>cmlb</b> v<code>" +
                std::string{cmlb::version} +
                "</code>";
            auto send = co_await deps_.messenger.send_html(req.chat, body);
            if (!send) co_return std::unexpected(send.error());
            co_return Result<void>{};
    ...
    // -------- ping --------------------------------------------------------
    register_("ping", Permission::Anyone,
        ...
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            ...
            auto edit = co_await deps_.messenger.edit_html(
                req.chat, *send,
                "<b>Pong!</b> Latency: <code>" +
                    std::to_string(latency.count()) + " ms</code>");
            if (!edit) co_return std::unexpected(edit.error());
            co_return Result<void>{};
        });

    // -------- log ---------------------------------------------------------
    register_("log", Permission::Owner,
        ...
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            ...
            if (ec || !exists) {
                auto send = co_await deps_.messenger.send_html(
                    req.chat,
                    "<b>Log:</b> <code>logs/cmlb.log</code> not found.");
                (void)send;
                co_return cmlb::core::error(
                    ErrorCode::Io, "log file not found: " + log_path.string());
            }
            ...
            if (bytes > kLogSizeCap) {
                ...
                (void)send;
                co_return cmlb::core::error(
                    ErrorCode::InvalidArgument, "log file too large to send");
            }
            auto send = co_await deps_.messenger.send_file(
                req.chat, log_path, /*caption=*/"cmlb.log",
        ...
        [this](CommandRequest req) -> asio::awaitable<Result<void>> {
            ...
            if (sub == "add") {
                if (split.tail.empty()) {
                    auto send = co_await deps_.messenger.send_html(
                        req.chat,
                        "<b>Usage:</b> <code>/rss add &lt;url&gt;</code>");
                    (void)send;
                    co_return cmlb::core::error(
                        ErrorCode::InvalidArgument, "/rss add requires a url");
                }
                ...
                if (!add_result) co_return std::unexpected(add_result.error());
                auto send = co_await deps_.messenger.send_html(
                    req.chat,
                    "<b>RSS:</b> Subscription added (id <code>" +
                        std::to_string(*add_result) + "</code>).");
                if (!send) co_return std::unexpected(send.error());
                co_return Result<void>{};
            }
            if (sub == "list") {
                auto list = co_await deps_.rss.list_for_chat(req.chat);
                if (!list) co_return std::unexpected(list.error());
            ...
            if (sub == "remove" || sub == "delete" || sub == "del") {
                ...
                if (id_split.head.empty()) {
                    auto send = co_await deps_.messenger.send_html(
                        req.chat,
                        "<b>Usage:</b> <code>/rss remove &lt;id&gt;</code>");
                    (void)send;
                    co_return cmlb::core::error(
                        ErrorCode::InvalidArgument,
                ...
                if (parse_failed) {
                    auto send = co_await deps_.messenger.send_html(
                        req.chat,
                        "<b>RSS:</b> Feed id must be a number.");
                    (void)send;
                    co_return cmlb::core::error(
                        ErrorCode::InvalidArgument, "feed id is not a number");
                }
                auto rem = co_await deps_.rss.remove(feed_id, req.sender, req.chat);
                if (!rem) co_return std::unexpected(rem.error());
                auto send = co_await deps_.messenger.send_html(
            ...
            (void)send;
            co_return cmlb::core::error(
                ErrorCode::InvalidArgument, "unknown rss subcommand");
        });
}
