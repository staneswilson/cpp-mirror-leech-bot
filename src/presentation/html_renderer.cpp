...
[[nodiscard]] std::string_view kind_label(cmlb::domain::TaskKind kind) noexcept {
    switch (kind) {
        case cmlb::domain::TaskKind::Mirror: return "Mirror";
        case cmlb::domain::TaskKind::Leech:  return "Leech";
        case cmlb::domain::TaskKind::Clone:  return "Clone";
    }
    return "Task";
}
...
std::string HtmlRenderer::render_task_status(
    ...
    const double fraction = (status.total_bytes > 0)
        ? static_cast<double>(status.downloaded_bytes)
              / static_cast<double>(status.total_bytes)
        : 0.0;

    const std::string name = escape_html(status.name.empty()
        ? task.metadata().source_url
        : status.name);
...
std::string HtmlRenderer::render_status(
    ...
    for (std::size_t i = 0; i < shown; ++i) {
        ...
        const double fraction = (s.total_bytes > 0)
...
std::string HtmlRenderer::render_stats(
    const cmlb::infrastructure::system::SystemSnapshot& metrics,
    std::chrono::seconds                                bot_uptime,
...
std::string HtmlRenderer::render_help(
    std::span<const CommandDescription> commands) {
    std::string out{"<b>Available commands</b>\n"};
    for (const auto& cmd : commands) {
        out.append(fmt::format(
            "<code>/{}</code> <i>({})</i>",
            cmd.name,
            cmlb::domain::to_string(cmd.permission)));
...
std::string HtmlRenderer::truncate_for_display(std::string_view text,
...
std::string HtmlRenderer::render_greeting(std::string_view user_first_name) {
    const std::string who = user_first_name.empty()
        ? std::string{"there"}
        : escape_html(user_first_name);
    return fmt::format(
        "<b>Hello, {}!</b>\n"
