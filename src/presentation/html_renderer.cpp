...
[[nodiscard]] std::string render_metrics_footer(
    ...
    return fmt::format("{}\n"
                       ...
                       HtmlRenderer::render_heading("System"),
...
std::string HtmlRenderer::render_heading(std::string_view text) {
    return fmt::format("<b><u>{}</u></b>", escape_html(text));
}

std::string HtmlRenderer::render_code(std::string_view text) {
    return fmt::format("<code>{}</code>", escape_html(text));
}

std::string HtmlRenderer::render_quote(std::string_view text) {
    return fmt::format("<blockquote>{}</blockquote>", escape_html(text));
}

std::string HtmlRenderer::render_usage(std::string_view command, std::string_view syntax) {
    std::string_view normalized = command;
    if (!normalized.empty() && normalized.front() == '/') {
        normalized.remove_prefix(1);
    }
    return fmt::format(
        "{}\n{}", render_heading("Usage"), render_code(fmt::format("/{} {}", normalized, syntax)));
}

std::string HtmlRenderer::render_success(std::string_view title, std::string_view detail) {
    if (detail.empty()) {
        return render_heading(title);
    }
    return fmt::format("{}\n{}", render_heading(title), render_quote(detail));
}

std::string HtmlRenderer::render_notice(std::string_view title, std::string_view detail) {
    if (detail.empty()) {
        return render_heading(title);
    }
    return fmt::format("{}\n{}", render_heading(title), render_quote(detail));
...
std::string HtmlRenderer::render_task_status(
    ...
    const std::string name = status.name.empty() ? task.metadata().source_url : status.name;
    ...
    return fmt::format("{}\n"
                       ...
                       "<b>State:</b> <code>{}</code>",
                       render_heading(kind_label(task.metadata().kind)),
                       render_quote(name),
...
std::string HtmlRenderer::render_status(
    ...
    std::string out = render_heading("Active Tasks");
    out.append("\n");
...
std::string HtmlRenderer::render_no_active_tasks(
    const cmlb::infrastructure::system::SystemSnapshot& metrics, std::chrono::seconds bot_uptime) {
    std::string out = render_heading("Status");
    out.append("\n");
    out.append(render_quote("No active tasks. New mirror and leech jobs will appear here."));
    out.append("\n\n");
    out.append(render_metrics_footer(metrics, bot_uptime));
    return out;
}
...
std::string HtmlRenderer::render_stats(const cmlb::infrastructure::system::SystemSnapshot& metrics,
    ...
    return fmt::format("{}\n"
                       ...
                       render_heading("Bot Statistics"),
                       render_quote("Live process and host-resource snapshot."),
                       active_downloads,
                       degraded_line,
                       format_duration(bot_uptime),
...
std::string HtmlRenderer::render_help(std::span<const CommandDescription> commands) {
    std::string out = render_heading("Available Commands");
    out.append("\n");
    out.append(render_quote("Only commands available to your permission level are shown."));
    out.push_back('\n');
...
std::string HtmlRenderer::render_user_settings(
    ...
    return fmt::format("{}\n"
                       ...
                       render_heading("User Settings"),
                       render_quote("Personal defaults used for new mirror and leech tasks."),
...
std::string HtmlRenderer::render_bot_settings(
    ...
    return fmt::format("{}\n"
                       ...
                       render_heading("Bot Settings"),
                       render_quote("Global controls for access, paths, limits, and polling."),
                       settings.owner_id,
...
std::string HtmlRenderer::render_failure(std::string_view stage,
    ...
    return fmt::format("{}\n"
                       ...
                       "<b>Reason:</b> {}\n"
                       "{}",
                       render_heading("Failed"),
                       escape_html(stage),
                       escape_html(friendly_label(error.code)),
                       render_quote(trimmed));
}
...
std::string HtmlRenderer::render_error(std::string_view task_name,
    ...
    return fmt::format("{}\n"
                       ...
                       "<b>Reason:</b> {}\n"
                       "{}",
                       render_heading("Error"),
                       escape_html(task_name),
                       escape_html(friendly_label(error.code)),
                       render_quote(truncate_for_display(error.message, 512)));
}

std::string HtmlRenderer::render_greeting(std::string_view user_first_name) {
    const std::string who =
        user_first_name.empty() ? std::string{"there"} : std::string{user_first_name};
    return fmt::format("{}\n"
                       "{}\n\n"
                       ...
                       render_heading(fmt::format("Hello, {}!", who)),
                       render_quote("High-speed mirror and leech orchestration for Telegram."));
}
