...
std::string format_binary(std::int64_t bytes, std::string_view suffix) {
    ...
    const auto abs_bytes =
        negative ? -static_cast<long double>(bytes) : static_cast<long double>(bytes);
    ...
    if (idx == 0) {
        return fmt::format(
            "{}{} {}{}", sign, static_cast<std::int64_t>(value), kUnits[idx].label, suffix);
    }
    return fmt::format(
        "{}{:.2f} {}{}", sign, static_cast<double>(value), kUnits[idx].label, suffix);
}
...
std::string render_progress_bar(double fraction, std::size_t width, char filled, char empty) {
...
std::string_view friendly_error_label(ErrorCode code) noexcept {
    ...
    case ErrorCode::JsonParse:
        return "Invalid JSON";
    case ErrorCode::TelegramApi:
        return "Telegram error";
    case ErrorCode::Aria2Rpc:
        return "aria2 error";
    case ErrorCode::QbittorrentApi:
        return "qBittorrent error";
    case ErrorCode::GoogleDriveApi:
        return "Google Drive error";
    case ErrorCode::RcloneInvocation:
        return "rclone error";
    ...
    case ErrorCode::SubprocessFailed:
        return "Helper process failed";
    case ErrorCode::MediaProcessing:
        return "Media processing failed";
    case ErrorCode::ArchiveProcessing:
        return "Archive processing failed";
