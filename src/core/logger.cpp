...
Result<spdlog::level::level_enum> parse_log_level(std::string_view text) {
    ...
    if (text == "warn"
        || text == "warning") return spdlog::level::warn;
    if (text == "error" || text == "err") return spdlog::level::err;
    if (text == "critical" || text == "fatal") return spdlog::level::critical;
...
Result<void> Logger::initialize(const LogConfig& config) {
    try {
        ...
        auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
            file_path.string(),
            config.rotating_file_max_bytes,
            config.rotating_file_max_files);
        ...
        return ::cmlb::core::error(ErrorCode::Internal, "spdlog initialization failed: unknown exception");
    }
}
