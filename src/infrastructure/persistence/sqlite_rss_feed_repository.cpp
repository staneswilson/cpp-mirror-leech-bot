...
boost::asio::awaitable<core::Result<std::vector<RssFeed>>> SqliteRssFeedRepository::list_enabled() {
...
boost::asio::awaitable<core::Result<std::optional<RssFeed>>> SqliteRssFeedRepository::find(
    std::int64_t feed_id) {
    auto acquired = co_await pool_.acquire();
    if (!acquired.has_value()) {
        co_return std::unexpected{acquired.error()};
...
boost::asio::awaitable<core::Result<RssFeed>> SqliteRssFeedRepository::save(RssFeed feed) {
    auto acquired = co_await pool_.acquire();
    if (!acquired.has_value()) {
        co_return std::unexpected{acquired.error()};
    }
    ...
    try {
        ...
        if (feed.feed_id == 0) {
            ...
            stmt << feed.title << feed.url << feed.chat.value() << feed.include_regex
                 << feed.exclude_regex << feed.last_guid;
            ...
            stmt << feed.feed_id << feed.title << feed.url << feed.chat.value()
                 << feed.include_regex << feed.exclude_regex << feed.last_guid;
            bind_optional_time(stmt, feed.last_checked_at);
            stmt << (feed.enabled ? 1 : 0) << to_iso8601(feed.created_at);
            stmt.execute();
        }
        ...
        if (code == SQLITE_CONSTRAINT || code == 2067 /* SQLITE_CONSTRAINT_UNIQUE */) {
            co_return core::error(core::ErrorCode::AlreadyExists,
...
boost::asio::awaitable<core::Result<void>> SqliteRssFeedRepository::update_state(
    std::int64_t feed_id,
    std::string last_guid,
...
boost::asio::awaitable<core::Result<void>> SqliteRssFeedRepository::remove(std::int64_t feed_id) {
    auto acquired = co_await pool_.acquire();
    if (!acquired.has_value()) {
        co_return std::unexpected{acquired.error()};
    }
    ...
    try {
        ...
        if (sqlite3_changes(db.connection().get()) == 0) {
            co_return core::error(core::ErrorCode::NotFound,
                                  "rss_feeds.remove: no row for feed_id "
                                      + std::to_string(feed_id));
        }
        co_return core::Result<void>{};
    } catch (const sqlite::sqlite_exception& ex) {
