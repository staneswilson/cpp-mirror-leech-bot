...
enum class MediaKind { Document, Video, Audio, Photo };
...
[[nodiscard]] std::string_view kind_label(MediaKind k) noexcept {
    switch (k) {
        ...
        case MediaKind::Photo:    return "photo";
...
[[nodiscard]] MediaKind classify(const fs::path& p) {
    static constexpr std::array<std::string_view, 5> video{
        ".mp4", ".mkv", ".mov", ".avi", ".webm"};
    static constexpr std::array<std::string_view, 6> audio{
        ".mp3", ".flac", ".wav", ".opus", ".m4a", ".aac"};
    static constexpr std::array<std::string_view, 4> photo{
        ".jpg", ".jpeg", ".png", ".webp"};

    const std::string ext = lower_ext(p);
    if (std::ranges::find(video, ext) != video.end()) return MediaKind::Video;
    if (std::ranges::find(audio, ext) != audio.end()) return MediaKind::Audio;
    if (std::ranges::find(photo, ext) != photo.end()) return MediaKind::Photo;
    return MediaKind::Document;
}

/// Splits `src` into `partNNN`-suffixed files of `chunk_bytes` each. Streams
/// in 1 MiB buffers. Returns the produced chunk paths.
[[nodiscard]] cmlb::core::Result<std::vector<fs::path>>
split_file(const fs::path& src, std::int64_t chunk_bytes) {
    std::ifstream in{src, std::ios::binary};
    if (!in) {
        return cmlb::core::error(cmlb::core::ErrorCode::Io,
                                 "telegram_uploader: cannot open "
                                     + src.string() + " for splitting");
    }
    ...
    if (ec) {
        return cmlb::core::error(cmlb::core::ErrorCode::FileSystem,
                                 "telegram_uploader: file_size failed: "
                                     + ec.message());
    }
    if (chunk_bytes <= 0) {
    ...
    const auto chunk_count = static_cast<int>(
        (total + static_cast<std::uintmax_t>(chunk_bytes) - 1)
    ...
    for (int i = 0; i < chunk_count; ++i) {
        fs::path part_path =
            src.string() + fmt::format(".part{:03d}", i + 1);
        std::ofstream out{part_path, std::ios::binary | std::ios::trunc};
        if (!out) {
            return cmlb::core::error(cmlb::core::ErrorCode::Io,
                                     "telegram_uploader: cannot create part "
                                         + part_path.string());
        }
...
double UploadProgress::progress_percent() const noexcept {
    ...
    const double pct = static_cast<double>(uploaded_bytes)
...
TelegramUploader::upload_file(fs::path path,
                              UploadConfig config,
    ...
    if (!fs::exists(path, ec) || ec) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::NotFound,
                                    "telegram_uploader: file not found: "
                                        + path.string());
    }
    ...
    const MediaKind kind = config.as_document ? MediaKind::Document
                                              : classify(path);
    cmlb::core::Logger::debug(
        "telegram_uploader: dispatch {} ({} bytes, kind={})",
        display_name, file_size, kind_label(kind));
    ...
    if (file_size <= split_bytes) {
        ...
        const std::string caption =
            config.caption.value_or(std::string{});
        auto msg_res = co_await messenger_.send_file(
            chat, path, caption, config.thumbnail_path);
        if (!msg_res) co_return std::unexpected(msg_res.error());
    ...
    if (!parts_res) co_return std::unexpected(parts_res.error());
    ...
    const int parallelism = std::min(
        std::max(configured_parallelism, 1), total_parts);

    {
        ...
        shared->done_timer =
            std::make_unique<asio::steady_timer>(coro_exec);
        shared->done_timer->expires_at(
            std::chrono::steady_clock::time_point::max());

        auto record_error =
            [shared](cmlb::core::AppError err) {
        ...
        for (int w = 0; w < parallelism; ++w) {
            asio::co_spawn(
                coro_exec,
                [shared, &messenger = messenger_, chat, total_parts,
                 file_size, display_name, thumb = config.thumbnail_path,
                 on_progress, record_error]() -> asio::awaitable<void> {
                    struct ExitGuard {
                        std::shared_ptr<SharedState> shared;
                        ~ExitGuard() noexcept {
                            if (shared->live.fetch_sub(
                    ...
                    for (;;) {
                        ...
                        if ((co_await asio::this_coro::cancellation_state)
                                .cancelled() != asio::cancellation_type::none) {
                        ...
                        if (idx >= total_parts) break;

                        const auto& part =
                            shared->part_paths[static_cast<std::size_t>(idx)];
                        std::error_code fec;
                        const auto part_size = static_cast<std::int64_t>(
                            fs::file_size(part, fec));
                        if (fec) {
                            record_error(cmlb::core::AppError{
                                cmlb::core::ErrorCode::FileSystem,
                                "telegram_uploader: file_size on part failed: "
                                    + fec.message()});
                            break;
                        }
                        shared->part_sizes[static_cast<std::size_t>(idx)] =
                            part_size;

                        const auto caption = fmt::format(
                            "Part{:02d}/{:02d} {}",
                            idx + 1, total_parts, display_name);

                        auto msg_res = co_await messenger.send_file(
                            chat, part, caption, thumb);
                        if (!msg_res) {
                            record_error(msg_res.error());
                            break;
                        }
                        shared->results[static_cast<std::size_t>(idx)] =
                            *msg_res;

                        const auto running = shared->uploaded_bytes.fetch_add(
                            part_size, std::memory_order_acq_rel) + part_size;

                        if (on_progress) {
                            const auto now = std::chrono::steady_clock::now();
                            std::unique_lock prog_lock{shared->progress_mu};
                            if (now - shared->last_cb
        ...
        struct PartsCleanup {
            ...
            ~PartsCleanup() noexcept {
                if (!armed || !parts) return;
                for (auto& p : *parts) {
                    std::error_code ec;
                    fs::remove(p, ec);
                    if (ec) {
                        cmlb::core::Logger::warn(
                            "telegram_uploader: failed to remove split part "
                            "{}: {}",
                            p.string(), ec.message());
                    }
                }
        ...
        co_await shared->done_timer->async_wait(
...
TelegramUploader::upload_directory(fs::path path,
    ...
    if (!fs::is_directory(path, ec) || ec) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::InvalidArgument,
                                    "telegram_uploader: not a directory: "
                                        + path.string());
    }
    ...
    for (auto it = fs::recursive_directory_iterator(
        ...
        if (ec) {
            co_return cmlb::core::error(
                cmlb::core::ErrorCode::FileSystem,
                "telegram_uploader: directory walk failed: " + ec.message());
        }
        if (it->is_regular_file(ec)) files.push_back(it->path());
    }
    std::ranges::sort(files);

    if (files.empty()) co_return std::vector<UploadResult>{};

    const int file_parallelism = std::clamp(
        telegram_config_.upload_files_parallelism, 1,
    ...
    for (int w = 0; w < file_parallelism; ++w) {
        asio::co_spawn(
            coro_exec,
            [this, shared, config, on_progress,
             record_error]() -> asio::awaitable<void> {
                ...
                while (!shared->abort.load(std::memory_order_acquire)) {
                    ...
                    const std::size_t idx = shared->next_index.fetch_add(
                        1, std::memory_order_acq_rel);
                    if (idx >= shared->files.size()) break;
                    auto res = co_await upload_file(
                        shared->files[idx], config, on_progress);
                    if (!res) {
