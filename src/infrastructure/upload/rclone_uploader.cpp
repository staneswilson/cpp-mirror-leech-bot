...
[[nodiscard]] double unit_to_factor(std::string_view unit) {
    ...
    if (unit == "TiB" || unit == "T")
        return 1024.0 * 1024 * 1024 * 1024.0;
    return 1.0;
}
...
[[nodiscard]] std::string join_remote(std::string_view base, std::string_view leaf) {
    std::string out{base};
...
[[nodiscard]] std::vector<std::string> build_common_rclone_args(
    const cmlb::core::RcloneConfig& cfg) {
    ...
    args.emplace_back("--multi-thread-streams=" + std::to_string(cfg.multi_thread_streams));
    ...
    if (cfg.use_mmap)
        args.emplace_back("--use-mmap");
    if (cfg.fast_list)
        args.emplace_back("--fast-list");
    if (cfg.drive_acknowledge_abuse)
        args.emplace_back("--drive-acknowledge-abuse");
...
[[nodiscard]] ParsedProgress parse_line(std::string_view line) {
    // Cheap rejection.
    if (line.find("Transferred:") == std::string_view::npos)
        return {};
    ...
    try {
        p.uploaded_bytes =
            static_cast<std::int64_t>(std::stod(m[1].str()) * unit_to_factor(m[2].str()));
        p.total_bytes =
            static_cast<std::int64_t>(std::stod(m[3].str()) * unit_to_factor(m[4].str()));
        p.speed_bps = static_cast<std::int64_t>(std::stod(m[5].str()) * unit_to_factor(m[6].str()));
        if (m[7].matched)
            p.eta = parse_eta(m[7].str());
        p.valid = true;
    } catch (...) {
        return {};
    }
...
boost::asio::awaitable<cmlb::core::Result<UploadResult>> RcloneUploader::run_rclone(
    ...
    if (!co_await probe_rclone()) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::RcloneInvocation,
                                    "rclone: binary unavailable (probe failed)");
    }
    if ((co_await asio::this_coro::cancellation_state).cancelled()
        != asio::cancellation_type::none) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::Cancelled, "rclone: cancelled");
    }
    ...
    req.on_stdout_line = [&](std::string_view line) {
        ...
        if (!on_progress)
            return;
        if ((now - last_cb) < std::chrono::seconds(1))
            return;
        last_cb = now;
        UploadProgress prog;
    ...
    if (res->exit_code != 0) {
        co_return cmlb::core::error(
            ...
            fmt::format("rclone: exited {}: {}", res->exit_code, res->stderr_data));
    }
...
boost::asio::awaitable<cmlb::core::Result<UploadResult>> RcloneUploader::upload_file(
    ...
    const std::string dest = join_remote(*config.rclone_path, path.filename().string());
...
RcloneUploader::upload_directory(fs::path path,
    ...
    if (!fs::is_directory(path, ec) || ec) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::InvalidArgument,
                                    "rclone: not a directory: " + path.string());
    }
    ...
    auto single =
        co_await run_rclone(std::move(args), path.filename().string(), std::move(on_progress));
    if (!single)
        co_return std::unexpected(single.error());
