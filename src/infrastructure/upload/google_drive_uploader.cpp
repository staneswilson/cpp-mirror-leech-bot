...
[[nodiscard]] cmlb::core::Result<std::string>
rs256_sign(std::string_view signing_input, std::string_view pem_private_key) {
    BIO* bio = BIO_new_mem_buf(pem_private_key.data(),
                               static_cast<int>(pem_private_key.size()));
    if (bio == nullptr) {
        return cmlb::core::error(cmlb::core::ErrorCode::Internal,
                                 "gdrive: BIO_new_mem_buf failed");
    }
    EVP_PKEY* pkey = PEM_read_bio_PrivateKey(bio, nullptr, nullptr, nullptr);
    BIO_free(bio);
    if (pkey == nullptr) {
        return cmlb::core::error(cmlb::core::ErrorCode::InvalidConfiguration,
                                 "gdrive: PEM_read_bio_PrivateKey failed: "
                                     + openssl_last_error());
    }
    ...
    if (ctx == nullptr) {
        EVP_PKEY_free(pkey);
        return cmlb::core::error(cmlb::core::ErrorCode::Internal,
                                 "gdrive: EVP_MD_CTX_new failed");
    }

    cmlb::core::Result<std::string> result =
        cmlb::core::error(cmlb::core::ErrorCode::Internal,
                          "gdrive: signing did not produce output");

    do {
        if (EVP_DigestSignInit(ctx, nullptr, EVP_sha256(), nullptr, pkey) <= 0) {
            result = cmlb::core::error(cmlb::core::ErrorCode::Internal,
                                       "gdrive: EVP_DigestSignInit failed: "
                                           + openssl_last_error());
            break;
        }
        if (EVP_DigestSignUpdate(ctx, signing_input.data(),
                                 signing_input.size()) <= 0) {
            result = cmlb::core::error(cmlb::core::ErrorCode::Internal,
                                       "gdrive: EVP_DigestSignUpdate failed: "
                                           + openssl_last_error());
            break;
        }
        ...
        if (EVP_DigestSignFinal(ctx, sig.data(), &sig_len) <= 0) {
            result = cmlb::core::error(cmlb::core::ErrorCode::Internal,
                                       "gdrive: EVP_DigestSignFinal failed: "
                                           + openssl_last_error());
            break;
        }
        sig.resize(sig_len);
...
[[nodiscard]] cmlb::core::Result<ServiceAccountKey>
load_service_account(const fs::path& credentials_path) {
    ...
    try {
        ...
        return cmlb::core::error(
            cmlb::core::ErrorCode::InvalidConfiguration,
...
header_lookup(const HttpResponse& res, std::string_view name) {
    for (const auto& h : res.headers) {
        if (h.name.size() == name.size()
            && std::ranges::equal(h.name, name, [](char a, char b) {
...
GoogleDriveUploader::GoogleDriveUploader(cmlb::core::Executor& exec,
    ...
    if (!loaded) {
        cmlb::core::Logger::error("gdrive: credentials unavailable: {}",
...
boost::asio::awaitable<cmlb::core::Result<DriveAccessToken>>
GoogleDriveUploader::mint_bearer() {
    auto jwt = build_signed_jwt();
    if (!jwt) co_return std::unexpected(jwt.error());
    ...
    try {
        ...
        tok.value      = "Bearer " + at;
        tok.expires_at = std::chrono::steady_clock::now()
                         + std::chrono::seconds(expires);
        co_return tok;
    } catch (const std::exception& e) {
        co_return cmlb::core::error(
            cmlb::core::ErrorCode::JsonParse,
            std::string{"gdrive: token response parse failed: "} + e.what());
    }
}

boost::asio::awaitable<cmlb::core::Result<std::string>>
GoogleDriveUploader::acquire_bearer() {
    {
    ...
    if (!fresh) co_return std::unexpected(fresh.error());
    {
        std::lock_guard lk{token_mutex_};
        cached_token_ = *fresh;
        co_return cached_token_.value;
...
GoogleDriveUploader::upload_multipart(fs::path path,
    ...
    if (!bearer) co_return std::unexpected(bearer.error());
    ...
    add_header(req, "Content-Type",
               fmt::format("multipart/related; boundary={}", boundary));
    ...
    if (res->status_code / 100 != 2) {
        co_return cmlb::core::error(
            ...
            fmt::format("gdrive: multipart upload returned {}: {}",
                        res->status_code, res->body));
    }
...
boost::asio::awaitable<cmlb::core::Result<UploadResult>>
GoogleDriveUploader::upload_resumable(fs::path path,
                                      const std::string& parent_folder,
                                      UploadProgressHandler on_progress) {
    namespace asio = boost::asio;
    const auto started = std::chrono::steady_clock::now();
    ...
    if (init_res->status_code / 100 != 2) {
        co_return cmlb::core::error(
            ...
            fmt::format("gdrive: resumable init returned {}: {}",
                        init_res->status_code, init_res->body));
    }
    ...
    std::int64_t chunk_bytes =
        static_cast<std::int64_t>(config_.chunk_size);
    if (chunk_bytes < kResumableAlignment) chunk_bytes = kResumableAlignment;
    chunk_bytes =
        (chunk_bytes / kResumableAlignment) * kResumableAlignment;
    ...
    plan.reserve(static_cast<std::size_t>((total + chunk_bytes - 1)
                                          / chunk_bytes));
    for (std::int64_t off = 0; off < total; off += chunk_bytes) {
        plan.push_back(ChunkPlan{off, std::min<std::int64_t>(chunk_bytes,
                                                             total - off)});
    }
...

    {
        ...
        auto record_error = [shared](cmlb::core::AppError err) {
            ...
            if (!shared->first_error) shared->first_error = std::move(err);
        ...
        auto worker_factory =
            [this, shared, plan_ptr, session_uri, total,
             path_str, record_error]() -> asio::awaitable<void> {
            ...
            struct ExitGuard {
                ...
                ~ExitGuard() noexcept {
                    if (shared->live_workers.fetch_sub(
            ...
            if (!local) {
                bail(cmlb::core::AppError{
                    cmlb::core::ErrorCode::Io,
                    "gdrive: worker cannot open " + path_str});
            } else {
                while (!shared->abort.load(std::memory_order_acquire)) {
                    if ((co_await asio::this_coro::cancellation_state)
                            .cancelled()
                        != asio::cancellation_type::none) {
                        shared->abort.store(true, std::memory_order_release);
                        break;
                    }
                    const int idx = shared->next_index.fetch_add(
                        1, std::memory_order_acq_rel);
                    if (idx >= static_cast<int>(plan_ptr->size())) break;
                    ...
                    for (int attempt = 0;
                         attempt <= max_retries && !chunk_done;
                         ++attempt) {
                        if (attempt > 0) {
                            const auto shift = std::min(attempt - 1, 10);
                            const auto delay = config_.initial_retry_delay
                                               * (1ULL << shift);
                            asio::steady_timer backoff{
                        ...
                        std::optional<cmlb::core::Result<
                            cmlb::infrastructure::http::HttpResponse>> chunk_res;
                        for (int auth_attempt = 0;
                             auth_attempt < 2;
                             ++auth_attempt) {
                            ...
                            auto chunk = make_request(HttpMethod::Put,
                                                     session_uri);
                            add_header(chunk, "Authorization", *bearer_now);
                            add_header(chunk, "Content-Length",
                                       std::to_string(got));
                            add_header(chunk, "Content-Range",
                                       fmt::format("bytes {}-{}/{}",
                                                   cp.offset,
                                                   cp.offset + got - 1, total));
                            chunk.body = body_buffer;

                            chunk_res =
                                co_await http_.request(std::move(chunk));
                            if (!chunk_res->has_value()) break;
                        ...
                        if (!chunk_res->has_value()) {
                            ...
                            if (chunk_res->error().code
                                == cmlb::core::ErrorCode::Cancelled) {
                        ...
                        if (code == 429 || (code >= 500 && code < 600)) {
                            ...
                            chunk_err = cmlb::core::AppError{
                                cmlb::core::ErrorCode::GoogleDriveApi,
                                fmt::format(
                                    "gdrive: chunk PUT exhausted {} retries: "
                                    "HTTP {}: {}",
                                    max_retries + 1, code, chunk_body)};
                            chunk_done = true;
                            break;
                        }
                        chunk_done = true;
                    ...
                    if (code == 200 || code == 201) {
                        ...
                        shared->bytes_uploaded.fetch_add(
                            got, std::memory_order_acq_rel);
                        continue;
                    }
                    if (code != 308) {
                        bail(cmlb::core::AppError{
                            cmlb::core::ErrorCode::GoogleDriveApi,
                            fmt::format("gdrive: chunk PUT returned {}: {}",
                                        code, chunk_body)});
                        break;
                    }
                    shared->bytes_uploaded.fetch_add(
        ...
        co_await shared->join_timer.async_wait(
...
GoogleDriveUploader::abort_resumable_session(std::string session_uri) {
    auto bearer = co_await acquire_bearer();
    ...
    if (!res) {
        cmlb::core::Logger::warn(
            "gdrive: aborting resumable session failed: {}",
            res.error().message);
    } else if (res->status_code / 100 != 2 && res->status_code != 499) {
        cmlb::core::Logger::warn(
            "gdrive: aborting resumable session returned {}",
            res->status_code);
    }
    co_return;
}
...
boost::asio::awaitable<cmlb::core::Result<std::string>>
GoogleDriveUploader::create_folder(std::string name, std::string parent) {
    auto bearer = co_await acquire_bearer();
    if (!bearer) co_return std::unexpected(bearer.error());
    ...
    if (res->status_code / 100 != 2) {
        co_return cmlb::core::error(
            cmlb::core::ErrorCode::GoogleDriveApi,
            fmt::format("gdrive: create folder returned {}: {}",
                        res->status_code, res->body));
    }
    try {
        json doc = json::parse(res->body);
...
boost::asio::awaitable<cmlb::core::Result<UploadResult>>
GoogleDriveUploader::upload_file(fs::path path,
                                 UploadConfig config,
                                 UploadProgressHandler on_progress) {
    namespace asio = boost::asio;
    ...
    const std::string parent =
        config.folder_id.value_or(config_.parent_folder_id);
    if (parent.empty()) {
        co_return cmlb::core::error(
    ...
    if (size <= kMultipartThreshold) {
        co_return co_await upload_multipart(std::move(path), parent,
                                            std::move(on_progress));
    }
    co_return co_await upload_resumable(std::move(path), parent,
                                        std::move(on_progress));
}
...
GoogleDriveUploader::upload_directory(fs::path path,
    ...
    auto root_create =
        co_await create_folder(path.filename().string(), root_parent);
    if (!root_create) co_return std::unexpected(root_create.error());
    ...
    for (auto it = fs::recursive_directory_iterator(
        ...
        if (it->is_directory(ec)) {
            ...
            const std::string& parent_id =
                (parent_it != folder_ids.end()) ? parent_it->second
                                                : folder_ids[fs::path{}];
            auto created = co_await create_folder(
                rel.filename().string(), parent_id);
    ...
    const int file_parallelism = std::clamp(
        config_.parallel_files_per_directory, 1,
        std::max(1, static_cast<int>(work.size())));
    ...
    auto file_worker =
        [this, shared, work_ptr, config, on_progress]() mutable
        -> asio::awaitable<void> {
        ...
        while (!shared->abort.load(std::memory_order_acquire)) {
            ...
            const std::size_t idx = shared->next_index.fetch_add(
                1, std::memory_order_acq_rel);
            if (idx >= work_ptr->size()) break;
...
/// as GoogleDriveApi.
[[nodiscard]] boost::asio::awaitable<cmlb::core::Result<json>>
fetch_metadata(BeastHttpClient& http,
    ...
    if (res->status_code == 404) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::NotFound,
                                    "gdrive: id not found: " + id);
    }
    if (res->status_code / 100 != 2) {
        co_return cmlb::core::error(
            cmlb::core::ErrorCode::GoogleDriveApi,
            fmt::format("gdrive: metadata fetch {} returned {}: {}",
                        id, res->status_code, res->body));
    }
    try {
...
[[nodiscard]] boost::asio::awaitable<cmlb::core::Result<std::vector<json>>>
list_children(BeastHttpClient& http,
              const std::string& bearer,
              const std::string& parent_id) {
    std::vector<json> out;
        ...
        if (res->status_code / 100 != 2) {
            co_return cmlb::core::error(
                ...
                fmt::format("gdrive: list children of {} returned {}: {}",
                            parent_id, res->status_code, res->body));
        }
        try {
            json doc = json::parse(res->body);
            ...
            if (doc.contains("nextPageToken")
                && doc["nextPageToken"].is_string()) {
...
boost::asio::awaitable<cmlb::core::Result<std::string>>
GoogleDriveUploader::copy(std::string source_id, std::string target_folder_id) {
    namespace asio = boost::asio;
    if (!ready_) {
    ...
    if (!meta) co_return std::unexpected(meta.error());
    ...
    if (mime != kFolderMime) {
        ...
        if (res->status_code / 100 != 2) {
            co_return cmlb::core::error(
                cmlb::core::ErrorCode::GoogleDriveApi,
                fmt::format("gdrive: copy returned {}: {}",
                            res->status_code, res->body));
        }
        try {
    ...
    if (!root_id) co_return std::unexpected(root_id.error());
    ...
    while (!stack.empty()) {
        if ((co_await asio::this_coro::cancellation_state).cancelled()
            != asio::cancellation_type::none) {
            co_return cmlb::core::error(cmlb::core::ErrorCode::Cancelled,
                                        "gdrive: copy cancelled");
        }
        Frame frame = std::move(stack.back());
        stack.pop_back();

        auto children = co_await list_children(http_, *bearer, frame.source);
        if (!children) co_return std::unexpected(children.error());
        ...
        for (const auto& child : *children) {
            ...
            if (cmime == kFolderMime) {
                auto created =
                    co_await create_folder(cname, frame.destination);
                if (!created) co_return std::unexpected(created.error());
                stack.push_back(Frame{cid, *created});
            } else {
                const json body = {{"parents",
                                    json::array({frame.destination})}};
                auto req = make_request(
                    HttpMethod::Post,
                    fmt::format(
...
boost::asio::awaitable<cmlb::core::Result<CountResult>>
GoogleDriveUploader::count(std::string folder_id) {
    namespace asio = boost::asio;
    if (!ready_) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::InvalidState,
                                    "gdrive: uploader not ready");
        ...
        for (const auto& child : *children) {
            ...
            if (cmime == kFolderMime) {
                ...
                if (child.contains("size") && child["size"].is_string()) {
                    try {
                        totals.total_bytes += std::stoll(
                            child["size"].get<std::string>());
                    } catch (...) {
...
GoogleDriveUploader::remove(std::string file_id) {
    if (!ready_) {
        co_return cmlb::core::error(cmlb::core::ErrorCode::InvalidState,
                                    "gdrive: uploader not ready");
    }
    ...
    if (res->status_code / 100 != 2) {
        co_return cmlb::core::error(
            cmlb::core::ErrorCode::GoogleDriveApi,
            fmt::format("gdrive: delete returned {}: {}",
                        res->status_code, res->body));
    }
