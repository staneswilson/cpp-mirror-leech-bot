...
SignalHandler::SignalHandler(asio::any_io_executor exec,
                             asio::cancellation_signal& shutdown)
    : signals_{exec, SIGINT, SIGTERM},
      shutdown_{shutdown} {
#if defined(SIGHUP)
    signals_.add(SIGHUP);
#endif
...
asio::awaitable<int> SignalHandler::wait_for_signal() {
    auto [ec, signum] = co_await signals_.async_wait(
