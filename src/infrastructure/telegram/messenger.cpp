...
Messenger::to_raw(const InlineKeyboard& kb) {
...
Messenger::send_html(domain::ChatId chat, std::string html) {
    co_return co_await gateway_.send_formatted_message(chat, std::move(html));
}

asio::awaitable<core::Result<void>>
Messenger::edit_html(domain::ChatId chat, domain::MessageId msg, std::string html) {
    co_return co_await gateway_.edit_formatted_message(chat, msg, std::move(html));
}

asio::awaitable<core::Result<domain::MessageId>>
Messenger::send_html_with_keyboard(domain::ChatId chat,
                                   std::string html,
                                   InlineKeyboard kb) {
...
asio::awaitable<core::Result<void>>
Messenger::edit_keyboard(domain::ChatId chat,
...
asio::awaitable<core::Result<void>>
Messenger::answer_callback(domain::CallbackQueryId id, std::string text, bool alert) {
    co_return co_await gateway_.answer_callback_query(id, std::move(text), alert);
}

asio::awaitable<core::Result<domain::MessageId>>
Messenger::send_file(domain::ChatId chat,
...
asio::awaitable<core::Result<void>>
Messenger::delete_message(domain::ChatId chat, domain::MessageId msg) {
...
InlineKeyboard Messenger::refresh_close_row(std::string refresh_data) {
    return InlineKeyboard{
        InlineKeyboardRow{
            InlineKeyboardButton{"Refresh", std::move(refresh_data)},
