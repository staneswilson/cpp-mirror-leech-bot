...
void UpdateRouter::rewire_gateway() {
    ...
    if (new_message_handler_) {
        bundle.on_new_message =
            [&exec = executor_, handler = new_message_handler_](
                domain::ChatId chat,
                    ...
                    [](std::exception_ptr eptr) {
                        if (eptr) {
                            try {
                                ...
                                core::Logger::error(
                                    "update_router: new-message handler threw: {}",
                                    e.what());
                            } catch (...) {
                                core::Logger::error(
                                    "update_router: new-message handler threw unknown");
                            }
                        }
                    });
            };
    ...
    if (callback_query_handler_) {
        bundle.on_callback_query =
            [&exec = executor_, handler = callback_query_handler_](
                domain::ChatId chat,
                domain::UserId sender,
                domain::MessageId msg_id,
