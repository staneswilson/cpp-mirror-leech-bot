...
AuthenticationFlow::AuthenticationFlow(TelegramGateway& gateway,
...
asio::awaitable<core::Result<void>> AuthenticationFlow::authenticate() {
    ...
    gateway_.set_auth_state_handler(
        [state, this](TelegramGateway::AuthState s) {
            state->last_state = s;
            switch (s) {
                case TelegramGateway::AuthState::WaitTdlibParameters:
                    ...
                    asio::co_spawn(
                        ...
                        [this]() -> asio::awaitable<void> {
                            ...
                            if (!r.has_value()) {
                                core::Logger::error(
                                    "auth: set_tdlib_parameters failed: {}",
                                    r.error().message);
                            }
                            co_return;
                        },
                ...
                case TelegramGateway::AuthState::Ready:
                    ...
                    asio::co_spawn(
                        ...
                        [this]() -> asio::awaitable<void> {
                            ...
                            if (!r.has_value()) {
                                core::Logger::warn(
                                    "auth: apply_runtime_options failed: {}",
                                    r.error().message);
                            }
                            co_return;
                        },
    ...
    co_await state->done.async_wait(
    ...
    if (!state->ready) {
        co_return core::error(
            core::ErrorCode::Unauthenticated,
