...
TEST_CASE("escape_html escapes the three significant characters",
          "[presentation][html]") {
    CHECK(HtmlRenderer::escape_html("a < b & c > d")
          == "a &lt; b &amp; c &gt; d");
    CHECK(HtmlRenderer::escape_html("plain text") == "plain text");
    CHECK(HtmlRenderer::escape_html("") == "");
}

TEST_CASE("render_task_status produces the expected fragments",
          "[presentation][html]") {
...
TEST_CASE("render_status with empty span returns the no-tasks placeholder",
    ...
    const auto html = HtmlRenderer::render_status({}, snapshot,
...
TEST_CASE("render_status with multiple tasks numbers and counts them",
          "[presentation][html]") {
    const auto snapshot = make_snapshot();
    ...
    const auto html = HtmlRenderer::render_status(
        downloads, snapshot, std::chrono::seconds{120});
...
TEST_CASE("render_status truncates excess tasks beyond the limit",
          "[presentation][html]") {
    std::vector<DownloadStatus> downloads;
    ...
    const auto html = HtmlRenderer::render_status(
        downloads, make_snapshot(), std::chrono::seconds{60});
    CHECK_THAT(html, ContainsSubstring("more task(s)"));
}

TEST_CASE("render_no_active_tasks always contains the footer",
          "[presentation][html]") {
    const auto html = HtmlRenderer::render_no_active_tasks(
        make_snapshot(), std::chrono::seconds{45});
    CHECK_THAT(html, ContainsSubstring("<b>No active tasks.</b>"));
...
TEST_CASE("render_stats reports active downloads and load averages",
          "[presentation][html]") {
    const auto html = HtmlRenderer::render_stats(
        make_snapshot(), std::chrono::seconds{125}, 7);
...
TEST_CASE("render_help renders each command name and permission",
          "[presentation][html]") {
...
TEST_CASE("render_user_settings renders every key field",
          "[presentation][html]") {
...
TEST_CASE("render_bot_settings renders the owner id and intervals",
          "[presentation][html]") {
...
TEST_CASE("render_error reports the task name, code, and message",
          "[presentation][html]") {
    const cmlb::core::AppError err{
        cmlb::core::ErrorCode::Network,
        "connection refused"};
...
TEST_CASE("render_error escapes HTML-special characters in inputs",
          "[presentation][html]") {
    const cmlb::core::AppError err{
