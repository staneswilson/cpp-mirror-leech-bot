...
const Action k_start_download    = [](Task& t) { return t.start_download(); };
const Action k_begin_processing  = [](Task& t) { return t.begin_processing(); };
const Action k_begin_upload      = [](Task& t) { return t.begin_upload(); };
const Action k_mark_completed    = [](Task& t) { return t.mark_completed(); };
const Action k_mark_failed       = [](Task& t) { return t.mark_failed("err"); };
const Action k_mark_cancelled    = [](Task& t) { return t.mark_cancelled(); };
...
TEST_CASE("mark_failed records the reason and is reachable from any non-terminal state",
          "[domain][task]") {
    const auto state = GENERATE(TaskState::Queued, TaskState::Downloading,
...
TEST_CASE("Invalid transitions yield InvalidState and preserve current state",
          "[domain][task]") {
