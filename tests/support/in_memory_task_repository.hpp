...
class InMemoryTaskRepository final : public cmlb::infrastructure::persistence::TaskRepository {
public:
    InMemoryTaskRepository() = default;
    ~InMemoryTaskRepository() override = default;

    boost::asio::awaitable<cmlb::core::Result<void>> save(cmlb::domain::Task task) override {
    ...
    boost::asio::awaitable<cmlb::core::Result<std::optional<cmlb::domain::Task>>> find(
        cmlb::domain::TaskId id) override {
        std::lock_guard lk{mutex_};
    ...
    boost::asio::awaitable<cmlb::core::Result<std::vector<cmlb::domain::Task>>> incomplete()
        override {
        std::lock_guard lk{mutex_};
    ...
    boost::asio::awaitable<cmlb::core::Result<std::vector<cmlb::domain::Task>>> for_user(
        cmlb::domain::UserId user) override {
        std::lock_guard lk{mutex_};
        std::vector<cmlb::domain::Task> out;
        for (const auto& [_, task] : store_) {
            if (task.metadata().user == user)
                out.push_back(task);
        }
    ...
    boost::asio::awaitable<cmlb::core::Result<void>> remove(cmlb::domain::TaskId id) override {
        std::lock_guard lk{mutex_};
        const auto erased = store_.erase(id);
        if (erased == 0) {
            co_return cmlb::core::error(cmlb::core::ErrorCode::NotFound, "task not found");
        }
        co_return cmlb::core::Result<void>{};
    }
