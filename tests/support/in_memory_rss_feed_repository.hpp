...
public:
    ...
    boost::asio::awaitable<
        cmlb::core::Result<std::optional<cmlb::infrastructure::persistence::RssFeed>>>
    find(std::int64_t feed_id) override {
        std::lock_guard lk{mutex_};
        auto it = store_.find(feed_id);
        if (it == store_.end()) {
            co_return std::optional<cmlb::infrastructure::persistence::RssFeed>{};
        }
        co_return std::optional<cmlb::infrastructure::persistence::RssFeed>{it->second};
    }

    boost::asio::awaitable<cmlb::core::Result<cmlb::infrastructure::persistence::RssFeed>> save(
        cmlb::infrastructure::persistence::RssFeed feed) override {
        std::lock_guard lk{mutex_};
        if (feed.feed_id == 0) {
            ...
            for (const auto& [_, existing] : store_) {
                if (existing.url == feed.url) {
                    co_return cmlb::core::error(cmlb::core::ErrorCode::AlreadyExists,
    ...
    boost::asio::awaitable<cmlb::core::Result<void>> update_state(
        std::int64_t feed_id,
        ...
        auto it = store_.find(feed_id);
        if (it == store_.end()) {
            co_return cmlb::core::error(cmlb::core::ErrorCode::NotFound, "feed not found");
        }
    ...
    boost::asio::awaitable<cmlb::core::Result<void>> remove(std::int64_t feed_id) override {
        std::lock_guard lk{mutex_};
