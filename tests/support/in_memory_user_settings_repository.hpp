...
public:
    ...
    boost::asio::awaitable<
        cmlb::core::Result<std::optional<cmlb::infrastructure::persistence::UserSettingsRecord>>>
    get(cmlb::domain::UserId user) override {
        std::lock_guard lk{mutex_};
        auto it = store_.find(user);
        if (it == store_.end()) {
            co_return std::optional<cmlb::infrastructure::persistence::UserSettingsRecord>{};
        }
        co_return std::optional<cmlb::infrastructure::persistence::UserSettingsRecord>{it->second};
    }

    boost::asio::awaitable<cmlb::core::Result<void>> save(
        cmlb::infrastructure::persistence::UserSettingsRecord record) override {
        std::lock_guard lk{mutex_};
    ...
    boost::asio::awaitable<cmlb::core::Result<void>> remove(cmlb::domain::UserId user) override {
        std::lock_guard lk{mutex_};
        if (store_.erase(user) == 0) {
            co_return cmlb::core::error(cmlb::core::ErrorCode::NotFound, "user settings not found");
        }
        co_return cmlb::core::Result<void>{};
    }
...
private:
    ...
    std::map<cmlb::domain::UserId, cmlb::infrastructure::persistence::UserSettingsRecord> store_;
};
