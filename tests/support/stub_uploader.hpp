...
class StubUploader final
    : public cmlb::infrastructure::upload::UploaderInterface {
public:
    struct FileCall {
    ...
    boost::asio::awaitable<cmlb::core::Result<
        cmlb::infrastructure::upload::UploadResult>>
    ...
    boost::asio::awaitable<cmlb::core::Result<
        std::vector<cmlb::infrastructure::upload::UploadResult>>>
    upload_directory(std::filesystem::path path,
                     cmlb::infrastructure::upload::UploadConfig config,
                     cmlb::infrastructure::upload::UploadProgressHandler)
        override {
        std::lock_guard lk{mutex_};
        directory_calls_.push_back(
            FileCall{std::move(path), std::move(config)});
        if (error_) {
            cmlb::core::AppError err = *error_;
            co_return std::unexpected(err);
        }
...
private:
    ...
    std::string                                            label_;
    cmlb::infrastructure::upload::UploadResult             next_result_{
        .file_id = "stub-id", .link = "stub://link", .size = 0,
