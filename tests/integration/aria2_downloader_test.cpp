...
TEST_CASE("aria2 downloader: connects to live daemon", "[integration][aria2][!mayfail]") {
    if (!e2e_enabled()) {
...
TEST_CASE("aria2 downloader: add_uri + status round-trip", "[integration][aria2][!mayfail]") {
    if (!e2e_enabled()) {
        SKIP("CMLB_E2E_ARIA2=1 not set");
    }
    ...
    const std::string url =
        getenv_or("CMLB_E2E_ARIA2_TEST_URL", "https://www.google.com/robots.txt");
    ...
    auto fut = boost::asio::co_spawn(
        ...
        [&]() -> boost::asio::awaitable<bool> {
            ...
            for (int i = 0; i < 100; ++i) {
                ...
                if (!status_res)
                    co_return false;
                if (status_res->state == DownloadState::Complete)
                    co_return true;
                if (status_res->state == DownloadState::Error)
                    co_return false;
                co_await boost::asio::steady_timer{co_await boost::asio::this_coro::executor, 100ms}
